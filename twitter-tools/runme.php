...
include('con.php');
//$query = "SELECT `Twitter Profile` FROM `DoubleThink` WHERE  `processed` !=1 LIMIT 0, 2000";
//$query = "SELECT `Twitter Profile` FROM `DoubleThink` LIMIT 0, 10";
//$query = "SELECT `FLIPTOP-Twitter` FROM `DoubleThink` WHERE `processed` != 1 LIMIT 0, 30";
//$query = "SELECT `KC-Twitter`  FROM `DoubleThink` WHERE `processed` = 0 LIMIT 0, 200";
$query = "SELECT `KC-Twitter`  FROM `DoubleThink` WHERE `Valid` !=1 LIMIT 0, 300";
...
foreach ($values as $value) {
    if ($count < 100) {
      ...
      $query2 = "UPDATE `master_tw`.`DoubleThink` SET `processed` = '1' 
      WHERE `DoubleThink`.`KC-Twitter` LIKE '" . mysql_real_escape_string($value) . "'";
...
foreach ($handles as $handle_string) {
    ...
    foreach ($usercontent as $user) {
        ...
        include('con.php');
        $u = "UPDATE  `master_tw`.`DoubleThink` SET 
        	`Valid` = '1', 
        	...
        	WHERE `DoubleThink`.`KC-Twitter` LIKE '" . mysql_real_escape_string($screen_name) . "';";
...
echo "<http://twitter-tools.zgresources.com/download.php'>download to excel</a>";
